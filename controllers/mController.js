	...
	const mReg = (req, res) => {
		...
		if(error.length < 1) {
			// res.render('signup', {error, fn, ln, phone, cacStatus, username, pass1, pass2, image})
			...
			merchant.save((err) => {
					if(err) {
						// res.send("There was a problem saving the data");
						// console.log(err)
			...
			if (error.length < 1) {
				...
				product.save((err) => {
					if (err) {
						...
						req.flash('message', "Product successfully saved into the database");
		...
		if (!req.session.merchant_id && !req.session.username) {
			...
			Product.find({username:req.session.username}, (error, result) => {
				if (error) {
					...
					res.render('view_product', {result, username:req.session.username});
				}
			})
		}
	}
